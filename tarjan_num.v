...
Record env := Env {black : {set V}; stack : seq V; esccs : {set {set V}};
...
Definition add_stack x e :=
  Env (black e) (x :: stack e) (esccs e) (sn e).+1
      (finfun [eta num e with x |-> sn e]).
Definition add_black x e :=
  Env (x |: black e) (stack e) (esccs e) (sn e) (num e).
Definition add_sccs x e :=
   let (s2, s3) := split_after x (stack e) in
   Env (x |: black e) s3 ([set y in s2] |: esccs e)
...
Definition dfs1 (dfs : {set V} -> env -> nat * env) (x : V) e :=
    ...
    if m1 < sn e then (m1, add_black x e1) else (infty, add_sccs x e1).
...
Notation white_def := (fun e => ~: (gray e :|: black e)).
Fact white_key : unit. Proof tt.
Definition white := locked_with white_key white_def.
Lemma whiteE : white = white_def. Proof. exact: unlock. Qed.
...
Lemma white_blackF x e : x \in white e -> x \in black e = false.
Proof. by rewrite whiteE grayE !inE; case: (x \in black _). Qed.

Lemma white_stackF x e : x \in white e -> x \in stack e = false.
Proof.
by rewrite whiteE grayE !inE andbC; case: (x \in stack _); rewrite //= orNb.
Qed.

Lemma white_grayF x e : x \in white e -> x \in gray e = false.
Proof. by rewrite whiteE !inE; case: (x \in gray e). Qed.

Lemma gray_stack x e : x \in gray e -> x \in stack e.
Proof. by rewrite grayE !inE andbC; case: (x \in stack _). Qed.

Lemma stack_whiteF x e : x \in stack e -> x \in white e = false.
Proof. by rewrite whiteE grayE !inE => ->; case: (_ \in _). Qed.

Lemma black_whiteF x e : x \in black e -> x \in white e = false.
Proof. by rewrite whiteE grayE !inE => ->. Qed.

Lemma white_add_stack x e : white (add_stack x e) = white e :\ x.
Proof.
apply/setP=> y; rewrite whiteE grayE !inE /=.
by case: (_ \in _) (_ \in _) (_ == _) => [] [] [].
Qed.

Lemma gray_add_stack x e : x \in white e ->
  gray (add_stack x e) = x |: gray e.
Proof.
move=> x_white; apply/setP=> y; move: x_white; rewrite whiteE grayE !inE.
...
Lemma white_add_black e x : white (add_black x e) = white e :\ x.
Proof.
rewrite whiteE gray_add_black /=; apply/setP=> y; rewrite !inE.
...
case: path.splitP sb se_uniq; first by rewrite gray_stack.
...
Lemma white_add_sccs e x :
  let s := take (index x (stack e)) (stack e) in
  x \in gray e -> uniq (stack e) -> s \subset black e ->
  white (add_sccs x e) = white e.
Proof.
move=> /= x_gray se_uniq sb; rewrite whiteE gray_add_sccs //=.
...
Record wf_color e := WfEnv {
   wf_stack : stack e =i gray e :|: (black e :\: cover (esccs e));
...
| Color4White of x \in white e : color4_spec x e false false false false true
...
Lemma color4P x e : wf_color e ->
                   color4_spec x e (x \in black e) (x \in stack e)
                              (x \in cover (esccs e))
                              (x \in gray e) (x \in white e).
Proof.
move=> [/(_ x) s_def /subsetP /(_ x) /implyP].
move: s_def; rewrite whiteE grayE !inE.
...
by constructor; rewrite whiteE grayE !inE x_black x_stack.
Qed.
...
Lemma sccs_black x e : wf_color e -> x \in cover (esccs e) -> x \in black e.
Proof. by case/color4P. Qed.

Lemma add_stack_cwf x e : x \in white e -> wf_color e -> wf_color (add_stack x e).
Proof.
move=> x_white e_wf; split=> /=; do ?
  by [exact: wf_sccs|rewrite white_stackF ?wf_uniq].
...
have [->|neq_xy] //= := altP (y =P x); rewrite ?(andbT, andbF).
  case: path.splitP new_black s_uniq => //=; first by rewrite gray_stack.
...
move=> y_black; case: path.splitP s_uniq; first by rewrite gray_stack.
by move=> s1 s2 s_uniq y_in; apply: uniq_catRL.
Qed.

Inductive stack_spec x e : bool -> bool -> bool -> bool -> bool -> Type :=
| StackWhite of x \in white e : stack_spec x e true false false false false
...
Lemma stackP x e : wf_color e ->
  stack_spec x e (x \in white e) (x \in stack e) (x \in cover (esccs e))
...
Record wf_num e := WfNum {
   wf_num0      : forall x, (num e x == 0) = (x \in white e);
...
Lemma add_stack_nwf x e : x \in white e -> wf_color e ->
  wf_num e -> wf_num (add_stack x e).
Proof.
move=> x_w wf_c wf_e.
have [nEw nEc nLs snE pE] := wf_e.
split => [y|y|y||y z]; rewrite ?ffunE ?inE /=.
- rewrite white_add_stack; case: (y =P x) => [->|/eqP yDx]; rewrite !inE.
    by rewrite snE eqxx.
...
have [nEw nEc nLs snE pE] := wf_e.
split => //= [y|].
  by rewrite white_add_black !inE nEw; case: eqP => // ->; case: color4P x_g.
...
split => [y|y|y||y z]; rewrite ?ffunE ?(inE) /=.
- rewrite white_add_sccs ?wf_uniq // mem_rcons inE.
...
- case: (path.splitP (gray_stack x_g)) tsb pE (wf_uniq wf_c) =>
...
Lemma add_stack_gwf e w :
  access_to e [set w] -> wf_color e -> wf_num e -> w \in white e -> wf_graph e ->
...
Lemma add_stack_pre e w :
  access_to e [set w] -> wf_color e -> w \in white e -> wf_graph e ->
...
Lemma subenv_white e1 e2 : subenv e1 e2 -> white e2 \subset white e1.
Proof.
by move=> e12; rewrite whiteE setCS (subenv_gray e12) setUS ?subenv_black.
Qed.

Lemma new_white e1 e2 : wf_color e2 -> subenv e1 e2 ->
   white e1 :\: white e2 = black e2 :\: black e1.
Proof.
move=> e2_wf e12; rewrite whiteE -(subenv_gray e12) setDE setCK setIC -setDE.
...
Record post_dfs (roots : {set V}) (e e' : env) (m : nat) := PostDfs {
  ...
  post_white : white e' = white e :\: \bigcup_(x in roots) wreach e x;
...
Lemma dfs_is_correct dfs1 dfsrec (roots : {set V}) e :
  ...
  (forall x, x \in roots -> forall e1, white e1 \subset white e ->
...
case=> [[e1_wf e1_cwf e1_gwf Nbw1 sccs_e1] sube1 white1 m1_min].
have [//|s1 s1_def s1b] := subenv_stackP _ sube1.
have e1_uniq := wf_uniq e1_wf.
rewrite big_set1 in white1 m1_min.
case: post_dfs.
- by rewrite subenv_white.
...
move=> [e2_wf e2_cwf e2_gwf Nbw2 sccs_e2] sube2 white2 m2_min.
...
have split_wreach : \bigcup_(y in roots) wreach e y =
  ...
  apply/bigcupP/bigcupP=> - [] z z_roots zy; exists z => //; last first.
    ...
    by rewrite connect1 //= uv andbT; apply: subsetP uw; rewrite subenv_white.
  ...
  move: zt; rewrite (@eq_connect _ _ (wedge e1)); last first.
    by move=> u v /=; rewrite !inE /= white1 !inE andbA.
...
split => //; first exact: subenv_trans sube2.
  by rewrite white2 white1 split_wreach setDDl.
...
+ suff: y \notin white e1 by case: color4P.
  by rewrite white1 inE xy.
...
have sx_subscc : is_subscc edge [set y in rcons s x].
  ...
  by rewrite (subenv_gray_le sube1).
...
have succ_whiteF z : z \in successors x -> z \in white e1 = false.
  move=> z_successor; apply/negbTE.
  rewrite white1 white_add_stack !inE !negb_and !negbK.
...
case: ltnP => [m1_small|m1_big] //=.
  ...
  - split=> //; first exact: add_black_cwf.
    + split => //.
      * move=> y /=.
        by rewrite white_add_black wf_num0// subND1//; case: color4P x_gray.
    ...
    + move=> y; rewrite !inE white_add_black.
  ...
  - rewrite white_add_black big_set1 wreachex white1 white_add_stack.
...
have scc_max : gscc_of x \subset [set y in s2].
  ...
  have /or3P[] : [|| y' \in white e1, y' \in cover (esccs e1)
  ...
  - rewrite s_def -cat_rcons mem_cat (negPf y'Ns2) /= => y'_stack.
    ...
    have x'Nstack: x' \notin stack e.
      ...
      by rewrite cat_rcons -s_def wf_uniq.
    have s2_white: {subset s2 <= white e}.
      move=> z z_s2; have: z \notin cover (esccs e).
    ...
    rewrite !inE (@connect_trans _ _ x') //; last first.
       by rewrite connect1 /= ?s2_white.
    apply: connect_sub xx's2 => u v /= /andP[/s2_white uw uv].
    by rewrite connect1 /= ?uw.
have g1Nx : gray e1 :\ x = gray e.
  by rewrite (subenv_gray sube1) gray_add_stack // setU1K // white_grayF.
...
- split=> //.
  ...
  + split=> //=. (* TODO: generalize *)
    * move=> y; rewrite white_add_sccs// ?take_s//.
  ...
  + move=> y; rewrite !inE white_add_sccs ?take_s //.
...
- rewrite white_add_sccs ?take_s ?sb //.
  by rewrite white1 white_add_stack big_set1 wreachex setDDl.
...
apply: dfs_is_correct => //= x x_root.
  ...
  rewrite white_add_stack cardsDS ?sub1set // cards1 subn1.
...
move=> e1 white_e1; apply: IHn; rewrite -ltnS (leq_trans _ n_ge) //.
have /subset_leq_card := white_e1.
...
- do ![split=> //; rewrite /= ?cover0 ?gray0 ?set0D ?setU0 //];
  ...
  + by move=> x; rewrite ffunE whiteE gray0 !inE eqxx.
