...
Variable V : finType.
Variable sccseq : rel V -> seq (seq V).
Variable g : rel V.

Hypothesis uniq_flatten : uniq (flatten (sccseq g)).
Hypothesis all_in_flatten : forall v : V, v \in (flatten (sccseq g)).
Hypothesis class_symconnect :
  forall c, c \in sccseq g -> exists x, forall y, (y \in c) = symconnect g x y.

Lemma sccs_ex C : C \in sccs g -> exists2 c, C = [set x in c] & c \in sccseq g.
Proof.
...
Lemma sccsP c : c \in sccseq g -> [set x in c] \in sccs g.
Proof.
...
Lemma class_acyclicP c (C := [set x in c]) : c \in sccseq g ->
...
Definition sccs_acyclic := all [pred c | class_acyclic c] (sccseq g).
